...
function ENT:Exit(kill) --####### Get out @RononDex
	...
	if (IsValid(self.Pilot)) then
		...
		self.Pilot:SetPos(self.ExitPos or self:GetPos()+self:GetUp()*105+self:GetRight()*100+self:GetForward()*-80);
...
function ENT:Think()
	...
	if(self.Inflight) then
		...
		if(!self.TakeOff and !self.Land) then
			local a = math.Clamp(20 - self.Mass/1000,4,20);
		...
		if(!self.Tractored) then
			if(self.Inflight and IsValid(self.Pilot)) then
				...
				if(self.PilotOffset) then
					pos = pos + (self:GetRight() * self.PilotOffset.x + self:GetForward() * self.PilotOffset.y + self:GetUp() * self.PilotOffset.z);
				end
...
function ENT:PhysicsCollide(cdat, phys)
	if(!Should_Collisions or self.LastCollide > CurTime()) then return end;
	...
	if(self.LastCollide < CurTime()) then
		...
		local dmg = (cdat.OurOldVelocity:Length()*s*math.Clamp(mass, 0, 1000))/750;
		...
		/*
		local n = self:GetPos():GetNormal();
		local cn = cdat.HitNormal;
		
		if( (n.x <= cn.x + 0.3 or n.x >= cn.x - 0.3) or (n.y <= cn.y + 0.3 or n.y >= cn.y - 0.3) or (n.z <= cn.z + 0.3 or n.z >= cn.z-0.3)) then
